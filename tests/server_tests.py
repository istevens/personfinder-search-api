...
''' % (self.hostport, self.hostport, self.hostport, self.hostport), doc.content)
    ...
    def put_person(self, first, last, subdomain='haiti'):
        db.put(Person(
    ...
    def test_api_search_with_no_subdomain_returns_results(self):
        self.put_person('John', 'Smith', subdomain='haiti')
        doc = self.go('/api/search?q=smith')
        xml = ElementTree.fromstring(doc.content)
        persons = xml.findall('pfif:person')
        assert 1 == len(persons)

    def test_api_search_with_subdomain_returns_results(self):
        self.put_person('John', 'Smith', subdomain='haiti')
        doc = self.go('/api/search?q=smith&subdomain=haiti')
        xml = ElementTree.fromstring(doc.content)
        persons = xml.findall('pfif:person')
        assert 1 == len(persons)
